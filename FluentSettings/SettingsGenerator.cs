...
", Encoding.UTF8));
        ...
        private static void Generate(SourceProductionContext spc, PropInfo propInfo)
        {
            // Распаковываем
            ...
            var attrData = propSymbol.GetAttributes()
                .First(ad => ad.AttributeClass?.ToDisplayString() == "FluentSettings.LocalSettingAttribute");
            ...
            var autoGeneratedAttribute = "        [global::System.CodeDom.Compiler.GeneratedCode(" +
                                        $"\"{typeof(SettingsGenerator).FullName}\", " +
                                        $"\"{version}\")]";
            ...
            foreach (var fullAttr in propInfo.CopiedAttrs)
            {
            ...
            spc.AddSource($"{clsName}_{propName}_LocalSetting.g.cs", SourceText.From(sb.ToString(), Encoding.UTF8));
        }
